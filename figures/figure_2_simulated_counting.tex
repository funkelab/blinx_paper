\begin{figure*}

  \begin{panel}{(a)}{\textwidth}
    \hspace{-2mm}%
    \setlength\plotwidth{\textwidth}%
    \setlength\plotheight{36mm}%
    \input{figures/panels/sim_trace_n10.tikz}
    \vspace{-4mm}
  \end{panel}

  \begin{panel}{(b)}{\textwidth}
    \hspace{-2mm}
    \setlength\plotwidth{\textwidth}%
    \setlength\plotheight{36mm}%
    \input{figures/panels/sim_trace_n20.tikz}
    \vspace{-4mm}
  \end{panel}

  \begin{panel}{(c)}{0.45\textwidth}
    \setlength\plotwidth{72mm}%
    \setlength\plotheight{72mm}%
    \input{figures/panels/sim_counting_estimates.tikz}
  \end{panel}
  \hspace{1cm}
  \begin{panel}{(d)}{0.55\textwidth}
    \begin{panel}{}{\textwidth}
      \hspace{5mm}%
      \small%
      \setlength\plotwidth{65mm}%
      \setlength\plotheight{22mm}%
      \input{figures/panels/trace_var_length.tikz}%
    \end{panel}
    \begin{panel}{(e)}{\textwidth}
      \hspace{5mm}%
      \small%
      \setlength\plotwidth{65mm}%
      \setlength\plotheight{22mm}%
      \input{figures/panels/trace_var_snr.tikz}%
    \end{panel}
  \end{panel}

  \caption{
    \panelref{a, b} Traces simulated from $\n=10$ (a) or 20 (b) emitters, and a density plot of measured intensities.
    The inferred state and inferred model demonstrate accurate fitting of both the intensity and transition distributions.
    Inset: posterior distribution estimated by \ours.
    %
    \panelref{c} Estimated versus true \n, shown as posteriors for \ours
    (orange heatmap) and point estimates for \lbfcs (purple dots). \textit{colorbar} 
    shows average probability across 10 traces.
    \captbc
  }
\end{figure*}
\begin{figure*}
  \contcaption{
    \capcont
    %
    \panelref{d} As trace length increases, the variance of the \ours posterior
    decreases, with limited improvement above 4000 frames.
    %
    \panelref{e} At low signal-to-noise, \ours incorrectly estimates the maximum count tested 
    as the most likely.
  }
  \label{fig:results:sim_counting}

\end{figure*}
