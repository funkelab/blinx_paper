...
(SNR) needed to achieve an accurate count.
  ...
  $ \frac{(\bar{x}_{z=1} - \bar{x}_{z=0})}{\sqrt{\text{var}(x_{z=1}) + \text{var}(x_{z=0}))}}$, 
...
qPAINT requires a kinetic regime where blinking 
	%
	events are short and infrequent~\citep{jungmann_2016} (\figref{fig:results:qpaint_counting}a,b).
	% 
