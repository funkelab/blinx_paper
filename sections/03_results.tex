...
Running \ours as a forward model, traces were generated for counts N=1 to 30
	(\figref{X} A), with experimentally relevant parameters. 
	%
...
\ours successfully fit simulated traces with counts up to N=30 (\figref{X}).
	...
	despite the fact that not every state is visited \ie the model is not just counting the number of peaks in the intensity histogram \figref{X} A, B,.
...
qPAINT is based on an accurate measure of the average dark time between blinking events. 
	%
	As a result, this method operates in an entirely different kinetic regime than lbFCS, where blinking "on" events are short and infrequent (\figref{X}).
