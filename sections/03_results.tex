...
the camera parameters.
	...
	over-estimation of count (see \secref{discussion}, \secref{sup_note_1}). 
...
\ours accurately fit simulated time series with counts up to $\n=30$ (\figref{fig:results:sim_counting}c).
	...
	but failed to estimate higher counts (further analysis in \secref{sup_note_2}).
	%
...
information to the model than shorter series and increase counting accuracy.
	%
...
(SNR) needed to achieve an accurate count.
	...
	corresponds to the parameters previously used) \secref{sup_note_3}.
	%
...
%
	...
	%
	The parameter values included the range of experimentally determined kinetics:
...
The results can be divided into two regimes: $\pon < \poff$ and $\pon \geq \poff$.
	...
	further discussion in \secref{sup_note_4}).
	%
...
% Experimental
To validate our experimental ability to... \todo{} explain figure 3b in 2 sentences
...
qPAINT relies on the accurate measurement of the average dark time between blinking events~\citep{jungmann_2016} 
	...
	% 
	Incorporating this calibration as priors on the kinetic parameters, 
	...
	to underestimate the molecular count (See \secref{sup_note_6}).
	%
...
which provides nano-scale control over the number and location of emitters~\citep{rothemund_folding_2006, strauss_2018}.
	...
	traces and super-resolution confirmed counts (see \secref{sup_note_7})
