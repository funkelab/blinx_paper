...
the camera parameters.
	%
	\revise{We also placed a uniform prior on the number of emitters betweeen 1 and 35},
	
	flat uniform priors on the kinetic parameters, allowing the model 
	to fit blinking rates without external bias,
	%
	loose priors on the emission parameters (\rb, \re) to prevent 
...
(SNR) needed to achieve an accurate count.
	...
	%
	\revise{Becuase noise is a function of intensity, here we define SNR as between the first two states:} $ \frac{(z_{1} - z_{0})}{\text{std}(z_{1} - z_{0})}$, 
	\revise{which is effectively an upper bound of the SNR between all states.}
	%
	(See \secref{calc_snr} for a note on how SNR was calculated).
	%
