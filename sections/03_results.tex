...
we generated traces for counts $\n=1$ to 30 (\figref{fig:results:sim_counting}a,b).
	...
	and closely match those reported in literature \cite{stein_2021}.
	%
...
the camera parameters,
	...
	will be $\z{} \pm 1$. 
	%
	Empirically, fitting $n=1$ captures this transition, and provides a good
...
\ours accurately fit simulated traces with counts up to $\n=30$ (\figref{fig:results:sim_counting}c).
	...
	%
	The true count was the most likely estimate in 177/300 (59\%) of traces, and 
	within 1 of the true count in 267/300 (89\%) of all traces.
	%
	Importantly, in many cases \ours is able to fit the correct count despite the fact that 
	...
	%
	Providing this value to \lbfcs lead to a substantial recovery of performance.
	%
	Out of the box, the \lbfcs estimated count was within 1 of the correct count for 64/300 (21\%) of traces, 
	but after providing \re, this accuracy increased to 200/300 (67\%).
	%

% Longer traces lead to better fits
A longer trace would provide more 
...
(SNR) needed to achieve an accurate count.
	%
	We artificially increase the variance of the readout noise \camvar, 
	...
	%
	This is due to model adding states to compensate for the wide distribution of intensities  
