...
the underlying biology of a system.
  ...
  counting XXX\todo{insert the right word here} to tell monomers, dimers,
  trimers\todo{right word?}, and higher order oligomers apart.
  %
...
microscopy.
  ...
  super-resolution techniques.\todo{other reasons why super resolution is not
  always possible/desirable? imaging speed?}
...
continuous replenishment of fluorophores from solution~\citep{stehr_2021}.
  ...
  statistics, rather than the data directly.\todo{find a positive spin, \eg,
  ``In contrast to those methods, our solution...''}
  %
  Further, all of the methods mentioned provide a single estimate of molecular
  count. A well calibrated probabilistic estimate would provide.
...
time.
  ...
  seven\todo{better: say what parameters (emission, camera, kinetics)}
  parameters directly to the sequence of measurements, estimating a likelihood
  ...
  %
  Finally the counting ability of \ours is proven experimentally, by validating
  the estimated count with ground truth measured by super-resolution DNA-PAINT.
  \todo{avoid passive voice, say ``we proved the counting...''}
