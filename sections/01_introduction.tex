...
%
Within these complexes, units of interest are often separated by only a few
...
%
  Rather, we estimate the molecular count directly through observation of 
...
shot noise and lacks the precision for accurate estimation of molecular count.
...
%
Many methods leverage temporal information for a more accurate estimate of 
molecular count, for example counting of bleaching events~\citep{ulbrich_2007,
...
problem~\citep{rollins_stochastic_2015,nino_2017}.
  %
  These produce repeated transitions in intensity which allow for modeling 
  at the single-molecule level, but are limited by complex photophysical properties. 
...
% continuous replenishment of fluorophores from solution~\citep{stehr_2021}.
  ...
  count based on the frequency of blinking events. 
  % For example, if a blinking rate of one
  ...
  qPAINT is a relative counting method, and therefore is dependant on the quality of 
  the calibration.
  %
  % lbFCS
