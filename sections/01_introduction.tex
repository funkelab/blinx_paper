...
%
  ...
  of protein complexes such as X, which ...\todo{important here to bridge from counting molecules (fluorophores) over binding sites to whatever one is interested in (subcomplexes, peptide motives, amino acids)}
...
While these small structures can no longer be directly spatially resolved, many\todo{all?} super-resolution 
  ...
  \todo{ExM is also a way to get beyond the resolution limit}

% Related Work
% --------------
% Overview of other counting methods
Perhaps the simplest method of molecular counting is correlating fluorescent
    intensity with the number of labels\todo{labels, fluorophores, binding sites: let's decide on one and be consistent (I vote for binding sites)}, \ie,\todo{remember we have beautiful macros for \ie and \eg :)} the brighter it is, the more there are.
    %
    ...
    utilize random flucutations\todo{install a spell-checker ;)} in fluorescent intensity over time to estimate the molecular count.
...
In contrast, DNA-PAINT is a method of producing blinking fluorescence that is functionally
    ...
    and is able to count up to 6 molecules\todo{in a single diffraction limited spot}.
    % qPAINT
