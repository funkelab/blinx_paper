...
between resolution and counting.
  ...
  %
  \ours can accurately estimate molecular count in both regimes, either calibration-free in the slow 
  or with a calibration in the fast.
...
we identified several limitations.
  ...
  %
  Second the intensity model does not capture all of the noise
  in the system \revise{(see \ref{noise_inference}).}
  %
  \revise{%
  Third, because of the repeated gradient optimization, \ours requires more
compute than \lbfcs or qPAINT (see \ref{runtime}).}
  %
  However, \revise{none} of these are fatal, and specific implementation details 
  allow us to minimize their effects (see~\ref{limits}).
  %

% Assumptions
In this work, we used DNA-PAINT to test the performance of \ours 
due to its resistance to photobleaching and consistent, 
predictable blinking behavior \revise{(see \ref{dna-paint-concerns})}.
  %
  ...
  % 
  This means that observed intensity will scale
  linearly with the number of active emitters $z$ and 
  the blinking kinetics of one \revise{emitter}
  has no effect on the blinking kinetics of any other.
  %
  In practice, sub-units in close proximity might 
  interact and violate this assumption~\citep{helmerich_photoswitching_2022}. 
  %
  However, other methods like qPAINT, also make this assumption and have been shown to
  ...
  (\pon, \poff, \re), which allows us to model the summed behavior of all sub-units combined.
  %
  In other words, we assume that each sub-unit is
  interchangeable with any other within the same spot.
  %
  In some applications, where non-uniform behavior among subunits may occur 
  \revise{
    (i.e. in different local environments (\pon and \poff)~\citep{civitci_2020}, 
    or at different distances from the coverslip (\re),
    }
  we expect a decrease in model performance.
