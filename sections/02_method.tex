...
(\figref{fig:method:overview}c). \revise{added citations}
  \cite{pawely_three-dimensional,huang_video-rate_2013, laine_imaging_2021}
  %
  note: in order to maximize signal to noise, we pool the intensities of the pixels within an ROI around the spot.
  Therefore we do not explicitly model the PSF, but only care about the sum total of photos detected.
...
(\figref{fig:method:overview}c).
  ...
  \revise{(KL-divergence $<10^{-3}$, see \ref{poisson})} and therefore we
