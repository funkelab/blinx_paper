...
number of emitters given the intensity trace, \ie:
  ...
  %
  The whole \hmm is conditioned on several parameters \parameters, \eg, the
  photon emission rates and the transition rates between different states and
  factorizes as usual over frames:\todo{list \parameterse, \parametersc, and
  \parameterst instead of examples}
  %
  \begin{equation}
    p(\trace| \n, \parameters) =
      \prod_{t=1}^{T}
...
of our microscope system.
  %
  (Note that in the following we omit the subscript $t$ in this section for
  clarity.)
  %
...
using an EMCCD camera).
  %
  However, because the detector of our system is an sCMOS camera, both noise
  distributions must be accounted for~\citep{huang_video-rate_2013}.
  \todo{reword: support readout noise, move to its section}
...
\eqref{eq:method:measurement_transform} and approximation \eqref{8}, the
...
%
  ...
  noise, see~\eqref{6}). Since the two distributions are normal, we can rewrite
