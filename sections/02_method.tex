...
To accurately model the observed intensity from a microscope system with an sCMOS camera, 
  the readout noise from the detector, and the shot noise of photon count must both be accounted for~\cite{huang_video-rate_2013} \figref{fig:method:overview}.
...
\textbf{Emission shot noise}:
  ...
  distributed~\cite{mehta_poisson_2016} \figref{fig:method:overview}. In this system, there are two sources of emitted photons, a number of emitters in the active state $\z{}\re$,
...
% sequences for imager and docker
