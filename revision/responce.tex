...
What we did:
\begin{description}
    \item[$\bullet$] toned down "all prior methods provide a single estimate of count", we meant all paint-based methods
    \item[$\bullet$] re-wrote the section in the intro on Bayesian based methods, including all references suggested by the reviewer (+ a few more)
\end{description}
...
What we did
\begin{description}
    \item[$\bullet$] changed sentence about ignoring p(n) to say p(n) is application specific
...
For other traces, the model is less confident (shown in figure S?). 
For these traces the correct count (4) is still the highest probability (~0.7), 
but counts > 4 also have a significant probability. 
...
For example the trace shown in figure S?, is estimated to have a count of 3, with high probability. 
...
What we did:
\begin{description}
    \item[$\bullet$] added additional figure to SI, to give more context to the distribution shown in figure 4e.
\end{description}
