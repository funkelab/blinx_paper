...
\begin{description}
    ...
    \item[$\bullet$] added a few sentences to section 2.1.1 Intensity Model explaining 
...
Our model assumes that the $p_{on}$ of an emitter in a spot is equal to that of every other 
emitter in the same spot, and that the same is true for $p_{off}$. However, we do not assume 
that $p_{on} = p_{off}$ for any emitters. We also address this assumption in the discussion 
...
In Figure 3a, we evaluate the models performance for a wide variety of $p_{on}$ and $p_{off}$ 
combinations. Further, in section 3.3 qPAINT Kinetic Regime and Figure 4, we evaluate the model 
in the exact regime that the reviewer describes, where $p_{on}$ is much less than $p_{off}$. 
